...
void Table::Initialize(std::vector<std::vector<Element>> input) {
  ...
  // 將 elements_ 從 |input| 移轉到 |elements_|。
  {
    int y = 1;
    for (auto& row : input) {
      int x = 1;
...
/// @brief 選取表格的行範圍。
/// @param row_min 要選取的第一行。
/// @param row_max 要選取的最後一行。
/// @note 您可以使用負數索引從末尾選取。
...
/// @brief 選取表格的列範圍。
/// @param column_min 要選取的第一列。
/// @param column_max 要選取的最後一列。
/// @note 您可以使用負數索引從末尾選取。
...
/// @brief 將 `decorator` 應用於選取範圍。
/// 這會裝飾單元格、線條和角落。
/// @param decorator 要應用的裝飾器。
// NOLINTNEXTLINE
...
/// @brief 將 `decorator` 應用於選取範圍。
/// 這只會裝飾模數為 `modulo`，偏移量為 `shift` 的線條。
/// @param decorator 要應用的裝飾器。
/// @param modulo 要裝飾線條的模數。
/// @param shift 要裝飾線條的偏移量。
// NOLINTNEXTLINE
...
/// @brief 將 `decorator` 應用於選取範圍。
/// 這只會裝飾模數為 `modulo`，偏移量為 `shift` 的角落。
/// @param decorator 要應用的裝飾器。
/// @param modulo 要裝飾角落的模數。
/// @param shift 要裝飾角落的偏移量。
// NOLINTNEXTLINE
...
/// @brief 將 `decorator` 套用到選取範圍的邊框上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的左邊框上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的右邊框上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的上邊框上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的下邊框上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的分隔線上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的垂直分隔線上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 將 `decorator` 套用到選取範圍的水平分隔線上。
/// @param decorator 要套用的裝飾器。
...
/// @brief 在選取範圍周圍應用 `border`。
/// @param border 要應用的邊框樣式。
...
/// @brief 在選取範圍周圍套用 `border`。
/// @param border 要套用的邊框樣式。
/// @param decorator 要套用的裝飾器。
