...
static const std::string charset_horizontal[11] = {
    ...
    // Microsoft 终端通常使用不支持表示整个仪表盘的8个 Unicode 字符的字体。使用较少的字符作为回退。    " ", " ", " ", " ", "▌", "▌", "▌", "█", "█", "█",
#else
    " ", " ", "▏", "▎", "▍", "▌", "▋", "▊", "▉", "█",
#endif
    // 在模糊测试器设法得到以下情况时，额外的一个字符：
...
/// @brief 绘制一个向指定方向推进的高清进度条。
/// @param progress 要填充区域的比例。范围在 [0,1] 之间。
/// @param direction 进度条的推进方向。
/// @ingroup dom
Element gaugeDirection(float progress, Direction direction) {
  return std::make_shared<Gauge>(progress, direction);
}

/// @brief 绘制一个从左到右推进的高清进度条。
/// @param progress 要填充区域的比例。范围在 [0,1] 之间。
///
/// ### 示例
///
