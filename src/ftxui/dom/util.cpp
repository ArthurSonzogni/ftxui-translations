...
/// @brief Une décoration qui ne fait absolument rien.
/// @ingroup dom
...
/// @brief À partir d'un élément, appliquer un décorateur.
...
/// @brief Applique un décorateur à un élément.
/// @return l'élément décoré.
/// @ingroup dom
///
/// ### Exemple
...
/// La dimension minimale qui correspondra à l'élément donné.
/// @see Fixed
/// @see Full
Dimensions Dimension::Fit(Element& e, bool extend_beyond_screen) {
  ...
  while (status.need_iteration && status.iteration < max_iteration) {
    ...
    // Ne donne pas à l'élément plus d'espace qu'il n'en a besoin :
    ...
    // Augmente la taille de la boîte jusqu'à ce qu'elle convienne...
    box.x_max = std::min(e->requirement().min_x, fullsize.dimx);
    box.y_max = e->requirement().min_y;

    // ... mais ne dépasse pas la taille de l'écran :
    if (!extend_beyond_screen) {
      box.y_max = std::min(box.y_max, fullsize.dimy);
    }
  }
