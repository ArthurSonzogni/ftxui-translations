...
void Compute3(Global& global) {
  // Étape 1: Disposer tous les éléments en rangées:
  {
    Line line;
    int x = 0;
    for (auto& block : global.blocks) {
      // Est-ce que cela rentre à la fin de la rangée ?
      // Non ? Alors nous devons en commencer une nouvelle :
  ...
  // Étape 2: Définir les positions sur l'axe X.
  SetX(global);
  JustifyContent(global);  // Distribuer l'espace restant.

  // Étape 3: Définir les positions sur l'axe Y.
  SetY(global);
}
