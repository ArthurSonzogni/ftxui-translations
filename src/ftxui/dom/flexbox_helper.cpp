...
void Compute3(Global& global) {
  // Paso 1: Disponer todos los elementos en filas:
  {
    Line line;
    int x = 0;
    for (auto& block : global.blocks) {
      // ¿Encaja al final de la fila?
      // ¿No? Entonces necesitamos empezar una nueva:
  ...
  // Paso 2: Establecer posiciones en el eje X.
  SetX(global);
  JustifyContent(global);  // Distribuir el espacio restante.

  // Paso 3: Establecer posiciones en el eje Y.
  SetY(global);
}
