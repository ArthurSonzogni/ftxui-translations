...
// Teste que les caractères |c| sont trivialement convertis en |Event::Character(c)|.
...
TEST(Event, UTF8) {
  struct {
    ...
    bool valid;
      ...
      // Teste les limites entre les différentes régions légales
      // https://unicode.org/versions/corrigendum1.html
      // Limite entre le valide et l'invalide
      // {{0x7F}, true}, => Séquence spéciale.
      {{0x80}, false},
      // ---
      {{0xC1, 0x80}, false},
      {{0xC2, 0x7F}, false},
...
TEST(Event, Special) {
  ...
  struct {
    ...
    Event expected;
      ...
      // Flèche (mode curseur par défaut)
      {str(" [A"), Event::ArrowUp},
      ...
      // Flèche (mode curseur d'application)
      ...
      // Retour arrière et particularité pour:
      ...
      // Touches de fonction pour terminal virtuel:
      {str("\x1B[[A"), Event::F1},
      {str("\x1B[[B"), Event::F2},
      {str("\x1B[[C"), Event::F3},
      {str("\x1B[[D"), Event::F4},
      ...
      // Touches de fonction pour xterm-r5, xterm-r6, rxvt
      {str("\x1B[11~"), Event::F1},
      {str("\x1B[12~"), Event::F2},
      {str("\x1B[13~"), Event::F3},
      {str("\x1B[14~"), Event::F4},

      // Touches de fonction pour vt100
      {str("\x1BOt"), Event::F5},
      {str("\x1BOu"), Event::F6},
      {str("\x1BOv"), Event::F7},
      {str("\x1BOl"), Event::F8},
      ...
      // Touches de fonction pour scoansi
      {str("\x1B[M"), Event::F1},
      {str("\x1B[N"), Event::F2},
      {str("\x1B[O"), Event::F3},
      {str("\x1B[P"), Event::F4},
