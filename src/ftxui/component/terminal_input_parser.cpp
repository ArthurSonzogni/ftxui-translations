...
const std::map<std::string, std::string> g_uniformize = {
    // El terminal de Microsoft utiliza un carácter de nueva línea diferente para la tecla
    // de retorno. Esto también ocurre en Linux con el comando `bind`:
    // Ver https://github.com/ArthurSonzogni/FTXUI/issues/337
    // Aquí, uniformizamos el carácter de nueva línea a `\n`.
    {"\r", "\n"},

    // Ver: https://github.com/ArthurSonzogni/FTXUI/issues/508
    {std::string({8}), std::string({127})},

    // Ver: https://github.com/ArthurSonzogni/FTXUI/issues/626
    //
    // Dependiendo del modo de tecla de cursor (DECCKM), el terminal envía diferentes
    ...
    {"\x1BOB", "\x1B[B"},  // ABAJO
    {"\x1BOC", "\x1B[C"},  // DERECHA
    {"\x1BOD", "\x1B[D"},  // IZQUIERDA
    {"\x1BOH", "\x1B[H"},  // INICIO
    {"\x1BOF", "\x1B[F"},  // FIN

    // Secuencias comunes de Inicio/Fin de terminales y multiplexores.
    {"\x1B[1~", "\x1B[H"},  // INICIO
    {"\x1B[4~", "\x1B[F"},  // FIN

    // Variaciones en torno a las teclas FN.
    // Internamente, estamos usando:
    // vt220, xterm-vt200, xterm-xf86-v44, xterm-new, mgt, screen
    // Ver: https://invisible-island.net/xterm/xterm-function-keys.html

    // Para la consola del sistema operativo Linux (CTRL+ALT+FN), que no pertenece a ningún
    // estándar real.
    // Ver: https://github.com/ArthurSonzogni/FTXUI/issues/685
    {"\x1B[[A", "\x1BOP"},    // F1
    {"\x1B[[B", "\x1BOQ"},    // F2
    {"\x1B[[C", "\x1BOR"},    // F3
    {"\x1B[[D", "\x1BOS"},    // F4
...
void TerminalInputParser::Send(TerminalInputParser::Output output) {
  switch (output.type) {
    ...
    case RESYNC: {
      // Los bytes acumulados hasta ahora no pueden continuarse con el de
      // |position_|, que inicia una nueva secuencia. Emitir el prefijo truncado y
      // analizar de nuevo los bytes restantes.
...
// └────────┴────────┴────────┴────────┘
//
// Además, algunas secuencias son ilegales si existe una representación más corta del
// mismo punto de código.
TerminalInputParser::Output TerminalInputParser::ParseUTF8() {
  auto head = Current();
  unsigned char selector = 0b1000'0000;  // NOLINT

  // La parte que no es punto de código del primer byte.
  unsigned char mask = selector;

  // Encontrar el primer cero en el primer byte.
  ...
  // Acumular el valor del primer byte.
  auto value = uint32_t(head & ~mask);  // NOLINT

  // UTF8 inválido, con más de 5 bytes.
  ...
  // Comprobar codificación UTF8 sobrelarga.
  int extra_byte = 0;
...
TerminalInputParser::Output TerminalInputParser::ParseESC() {
  ...
  switch (Current()) {
    ...
    // No se permite un ESC dentro de una secuencia. Este inicia una nueva.
    case '\x1B':
      return RESYNC;

    // Se esperan 2 caracteres.
    case ' ':
    case '#':
    case '%':
    case '(':
    ...
    // Se espera 1 carácter:
    default:
      return SPECIAL;
  }
}

// ESC P ... ESC BARRA INCLINADA INVERSA
TerminalInputParser::Output TerminalInputParser::ParseDCS() {
  // Analizar hasta el terminador de cadena ST.
  while (true) {
    if (!Eat()) {
      return UNCOMPLETED;
    }
...
TerminalInputParser::Output TerminalInputParser::ParseCSI() {
  ...
  while (true) {
    ...
    // CSI termina con un carácter en el rango 0x40–0x7E
    // (ASCII @A–Z[\]^_`a–z{|}~),
    if (Current() >= '@' && Current() <= '~' &&
        // Nota: No recuerdo por qué excluimos '<'
        Current() != '<' &&
        // Para manejar F1-F4, excluimos '['.
        Current() != '[') {
      arguments.push_back(argument);
      argument = 0;  // NOLINT
    ...
    // ESC inválido en CSI. Inicia una nueva secuencia.
    if (Current() == '\x1B') {
      return RESYNC;
    }
  }
...
TerminalInputParser::Output TerminalInputParser::ParseMouse(  // NOLINT
  ...
  // Bits valor Modificador  Comentario
  // ---- ----- ------- ---------
  // 0 1  1 2   botón    0 = Izquierda, 1 = Medio, 2 = Derecha, 3 = Liberar
  // 2    4     Shift
  // 3    8     Meta
  // 4    16    Control
  // 5    32    Movimiento
...
TerminalInputParser::Output TerminalInputParser::ParseDeviceAttributes(
    ...
    std::vector<int> arguments) {
  if (altered_greater) {
    // Atributos de Dispositivo Secundarios (DA2)
    // ESC [ > Pp ; Pv ; Pc c
    if (arguments.size() >= 3) {
      // Pp: Tipo de terminal
      // Pv: Versión de firmware
      // Pc: Opciones de hardware
      ...
      // Caso especial para xterm, que a menudo devuelve 0;pv;0 o similar
      // pero no sigue estrictamente los tipos DEC VT.
      return output;
    }
  } else if (altered_question) {
    // Atributos de Dispositivo Primarios (DA1)
    // ESC [ ? Pp ; ... c
