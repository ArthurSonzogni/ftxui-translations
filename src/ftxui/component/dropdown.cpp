...
/// @brief Un menú desplegable.
/// @ingroup component
/// @param entries La lista de entradas a mostrar.
/// @param selected El índice de la entrada seleccionada.
...
/// @brief Un menú desplegable.
/// @ingroup component
/// @param option Las opciones para el menú desplegable.
// NOLINTNEXTLINE
Component Dropdown(DropdownOption option) {
  class Impl : public ComponentBase, public DropdownOption {
   public:
    ...
    // Cambiar el foco entre el checkbox y el radiobox al seleccionarlo.
    bool OnEvent(ftxui::Event event) override {
      const bool open_old = open_();
      const int selected_old = selected_();
      bool handled = ComponentBase::OnEvent(event);

      // Transferir el foco al radiobox cuando el menú desplegable se abre.
      if (!open_old && open_()) {
        radiobox_->TakeFocus();
      }

      // Cerrar automáticamente el menú desplegable cuando el usuario selecciona un elemento,
      // incluso si el elemento es el mismo que el anterior.
      if (open_old && open_()) {
