...
// @brief 模仿指數函數 y = 2^(10(x - 1))
float ExponentialIn(float p) {
  return (p == 0.f) ? p : std::pow(2.f, 10.f * (p - 1.f));
}

// @brief 模仿指數函數 y = -2^(-10x) + 1
...
// @brief 模仿阻尼正弦波 y = sin(13pi/2*x)*pow(2, 10 * (x -
// 1))
float ElasticIn(float p) {
  return std::sin(13.f * kPi2 * p) * std::pow(2.f, 10.f * (p - 1.f));
}

// @brief 模仿阻尼正弦波 y = sin(-13pi/2*(x + 1))*pow(2,
// -10x) +
// 1
...
// @brief 模仿過沖三次函數 y = x^3-x*sin(x*pi)
float BackIn(float p) {
  return p * p * p - p * std::sin(p * kPi);
}

// @brief 模仿過沖三次函數 y = 1-((1-x)^3-(1-x)*sin((1-x)*pi))
