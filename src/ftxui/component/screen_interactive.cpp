...
/// Crée un ScreenInteractive prenant toute la taille du terminal. Ceci utilise le
/// tampon d'écran alternatif pour éviter de perturber le contenu du terminal.
/// @note Ceci est identique à `ScreenInteractive::FullscreenAlternateScreen()`
// static
ScreenInteractive ScreenInteractive::Fullscreen() {
  return FullscreenAlternateScreen();
}

/// Crée un ScreenInteractive prenant toute la taille du terminal. Le tampon d'écran
/// principal est utilisé. Cela signifie que si le terminal est redimensionné, le contenu
/// précédent pourrait perturber le contenu du terminal.
// static
...
/// Crée un ScreenInteractive dont la largeur correspond à la largeur de sortie du terminal et
/// dont la hauteur correspond au composant dessiné.
// static
...
/// Crée un ScreenInteractive dont la largeur et la hauteur correspondent au composant
/// dessiné.
// static
...
/// @brief Définit si la souris est suivie et si les événements sont signalés.
/// appelé en dehors de la boucle principale. Par exemple `ScreenInteractive::Loop(...)`.
/// @param enable Indique s'il faut activer le suivi des événements de la souris.
/// @note Ceci doit être appelé en dehors de la boucle principale. Par exemple, avant d'appeler
/// `ScreenInteractive::Loop`.
/// @note Le suivi de la souris est activé par défaut.
/// @note Le suivi de la souris n'est pris en charge que sur les terminaux qui le supportent.
///
/// ### Exemple
///
/// ```cpp
...
/// @brief Active ou désactive la gestion automatique des entrées pipées.
/// Lorsqu'elle est activée, FTXUI détectera les entrées pipées et redirigera stdin depuis /dev/tty
/// pour l'entrée clavier, permettant aux applications de lire des données pipées tout en
/// recevant des événements clavier interactifs.
/// @param enable Indique s'il faut activer la gestion des entrées pipées. La valeur par défaut est true.
/// @note Ceci doit être appelé avant Loop().
/// @note Cette fonctionnalité est activée par défaut.
/// @note Cette fonctionnalité n'est disponible que sur les systèmes POSIX (Linux/macOS).
...
/// @brief Ajoute une tâche pour dessiner l'écran une fois de plus, jusqu'à ce que toutes les animations
/// soient terminées.
...
/// @brief Tente d'obtenir le verrou unique permettant de capturer la souris.
...
/// @brief Indique si la boucle principale a été quittée.
...
/// @brief Décore une fonction. Elle s'exécute de la même manière, mais avec les
/// hooks du terminal de l'écran actuellement actif temporairement désinstallés pendant son exécution.
/// @param fn La fonction à décorer.
...
/// @brief Force FTXUI à gérer ou non le Ctrl-C, même si le composant
/// intercepte l'événement Event::CtrlC.
void ScreenInteractive::ForceHandleCtrlC(bool force) {
  force_handle_ctrl_c_ = force;
}

/// @brief Force FTXUI à gérer ou non le Ctrl-Z, même si le composant
/// intercepte l'événement Event::CtrlZ.
...
/// @brief Retourne l'écran actuellement actif, ou null si aucun.
// static
...
/// @brief Retourne une fonction pour quitter la boucle principale.
