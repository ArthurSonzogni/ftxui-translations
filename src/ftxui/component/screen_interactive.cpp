...
/// 创建一个占据整个终端大小的 ScreenInteractive。这使用备用屏幕缓冲区，
/// 以避免弄乱终端内容。
/// @note 这与 `ScreenInteractive::FullscreenAlternateScreen()` 相同
// static
ScreenInteractive ScreenInteractive::Fullscreen() {
  return FullscreenAlternateScreen();
}

/// 创建一个占据整个终端大小的 ScreenInteractive。正在使用主屏幕缓冲区。
/// 这意味着如果终端被调整大小，之前的内容可能会与终端内容混淆。
// static
...
/// 创建一个 ScreenInteractive，其宽度与终端输出宽度匹配，
/// 高度与正在绘制的组件匹配。
// static
...
/// @brief 设置是否跟踪鼠标并报告事件。
/// 在主循环之外调用。例如 `ScreenInteractive::Loop(...)`。
/// @param enable 是否启用鼠标事件跟踪。
/// @note 这必须在主循环之外调用。例如在调用 `ScreenInteractive::Loop` 之前。
/// @note 鼠标跟踪默认启用。
/// @note 鼠标跟踪仅在支持的终端上受支持。
///
/// ### 示例
///
/// ```cpp
...
/// @brief 启用或禁用自动管道输入处理。
/// 启用后，FTXUI 将检测管道输入并将 stdin 从 /dev/tty 重定向
/// 以进行键盘输入，从而允许应用程序读取管道数据，同时仍
/// 接收交互式键盘事件。
/// @param enable 是否启用管道输入处理。默认为 true。
/// @note 这必须在 Loop() 之前调用。
/// @note 此功能默认启用。
/// @note 此功能仅在 POSIX 系统（Linux/macOS）上可用。
...
/// @brief 尝试获取能够捕获鼠标的唯一锁。
/// @return 如果鼠标尚未被捕获，则返回一个唯一锁，否则返回空。
