...
// 本源代码的使用受 MIT 许可证的管辖，该许可证可在 LICENSE 文件中找到。
#include <map>  // for map
#include <string>
...
// 禁用影子变量的警告，适用于所有编译器。实际上，每个字母都有一个静态事件：
#ifdef __clang__
...
/// @brief 对应于给定输入字符序列的事件。
/// @param input 终端发送的字符序列。
/// @param mouse 鼠标状态。
// static
...
/// @brief 一个自定义事件，其含义由库用户定义。
/// @param input 由开发者定义的任意字符序列。
// static
