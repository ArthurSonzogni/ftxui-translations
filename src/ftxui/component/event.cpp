...
// 禁用變數遮蔽警告，適用於所有編譯器。事實上，每個字母都有一個靜態 Event：
#ifdef __clang__
...
/// @brief 對應於終端機傳送的字元序列的事件。
/// @param input 終端機傳送的字元序列。
/// @param mouse 滑鼠狀態。
// static
...
/// @brief 一個自訂事件，其意義由函式庫的使用者定義。
/// @param input 由開發者定義的任意字元序列。
// static
...
// 我們遵循 xterm-new / vterm-xf86-v4 / mgt / screen
