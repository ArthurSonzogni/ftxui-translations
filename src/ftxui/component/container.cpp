...
/// @brief 一个组件列表，垂直逐个绘制，并使用上/下箭头键或“j”/“k”键垂直导航。
/// @param children 组件列表。
/// @ingroup component
/// @see ContainerBase
///
...
/// @brief 一个组件列表，水平逐个绘制，并使用左/右箭头键或“h”/“l”键水平导航。
/// @param children 组件列表。
/// @ingroup component
/// @see ContainerBase
///
...
/// @brief 一个组件列表，一次只绘制并与其交互一个。|selector| 提供所选组件的索引。这对于实现选项卡很有用。
/// @param children 组件列表。
/// @param selector 绘制的子组件的索引。
/// @ingroup component
/// @see ContainerBase
///
/// ### Example
...
/// @brief 一个组件列表，它们相互堆叠。
/// 事件传播到第一个组件，如果未处理，则传播到第二个，依此类推。
/// 组件以给定顺序的相反顺序绘制。
/// 当一个组件获得焦点时，它会被置于最前面，而不改变其他元素的相对顺序。
///
/// 这应该与 `Window` 组件一起使用。
///
/// @param children 组件列表。
