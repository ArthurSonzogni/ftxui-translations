...
// 从 UTF8 编码的字符串 |input| 中，提取 1 到 4 字节表示一个码点。
// 将码点放入 |ucs|。从 |start| 开始，并更新 |end| 以表示下一个要提取的字节的起始位置，
// 以便连续执行。
...
// 从 UTF16 编码的字符串 |input| 中，提取 1 到 4 字节表示一个码点。
// 将码点放入 |ucs|。从 |start| 开始，并更新 |end| 以表示下一个要提取的字节的起始位置，
// 以便连续执行。
bool EatCodePoint(const std::wstring& input,
  ...
  // 在 Windows 上，wstring 使用 UTF16 编码：
...
std::wstring to_wstring(const std::string& s) {
  ...
  uint32_t codepoint = 0;
  while (EatCodePoint(s, i, &i, &codepoint)) {
    // 在 Linux 上，wstring 采用 UTF32 编码：
    if constexpr (sizeof(wchar_t) == 4) {
      out.push_back(codepoint);  // NOLINT
      continue;
    }

    // 在 Windows 上，wstring 采用 UTF16 编码：
