...
#include <array>    // 用於 array
#include <cstddef>  // 用於 size_t
#include <cstdint>  // 用於 uint32_t, uint8_t, uint16_t, int32_t
#include <string>   // 用於 string, basic_string, wstring
#include <tuple>    // 用於 _Swallow_assign, ignore
#include <vector>

#include "ftxui/screen/deprecated.hpp"       // 用於 wchar_width, wstring_width
#include "ftxui/screen/string_internal.hpp"  // 用於 WordBreakProperty, EatCodePoint, CodepointToWordBreakProperty, GlyphCount, GlyphIterate, GlyphNext, GlyphPrevious, IsCombining, IsControl, IsFullWidth, Utf8ToWordBreakProperty
...
// 從 UTF8 編碼的字串 |input| 中，讀取介於 1 到 4 個位元組，表示一個碼點。
// 將碼點放入 |ucs| 中。從 |start| 開始，並更新 |end| 以表示下一個要讀取的位元組的開頭，
// 以便連續執行。
...
// 從 UTF16 編碼的字串 |input| 中，讀取介於 1 到 4 個位元組，表示一個碼點。
// 將碼點放入 |ucs| 中。從 |start| 開始，並更新 |end| 以表示下一個要讀取的位元組的開頭，
// 以便連續執行。
...
std::vector<std::string> Utf8ToGlyphs(const std::string& input) {
  ...
  while (start < input.size()) {
    ...
    // 全形字元佔用兩個單元格。第二個是空字串，用於保留第一個佔用的空間。
...
size_t GlyphNext(const std::string& input, size_t start) {
  ...
  while (start < input.size()) {
    ...
    // 我們處理下一個字形的開頭。如果我們正在處理請求的字形，立即返回其起始位置。
    if (glyph_found) {
