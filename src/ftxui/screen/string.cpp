...
// ソートされたIntervalのリスト内でコードポイントを検索します。
template <size_t N>
...
// ソートされたInterval + プロパティのリスト内で値を検索します。
...
// UTF8エンコードされた文字列|input|から、1つのコードポイントを表す1〜4バイトを読み取ります。// コードポイントを|ucs|に格納します。|start|から開始し、連続する実行のために次のバイトの// 開始位置を表すように|end|を更新します。
...
// UTF16エンコードされた文字列|input|から、1つのコードポイントを表す1〜4バイトを読み取ります。// コードポイントを|ucs|に格納します。|start|から開始し、連続する実行のために次のバイトの// 開始位置を表すように|end|を更新します。
bool EatCodePoint(const std::wstring& input,
  ...
  // LinuxではwstringはUTF32エンコーディングを使用します。
...
std::vector<std::string> Utf8ToGlyphs(const std::string& input) {
  ...
  while (start < input.size()) {
    ...
    // 全角文字は2つのセルを占めます。2番目のセルは、最初のセルが占めるスペースを確保するために// 空の文字列で構成されます。
    if (IsFullWidth(codepoint)) {
...
size_t GlyphNext(const std::string& input, size_t start) {
  ...
  while (start < input.size()) {
    ...
    // 次のグリフの開始を読み取ります。要求されたものを読み取っている場合は、// その開始位置を直ちに戻します。    if (glyph_found) {
      return static_cast<int>(start);
    }

    // それ以外の場合は、このグリフをスキップして反復します:
    glyph_found = true;
    start = end;
  }
...
int GlyphCount(const std::string& input) {
  ...
  while (start < input.size()) {
    ...
    // 結合文字を無視します。ただし、先行する結合文字がない場合は除きます。    if (IsCombining(codepoint)) {
      if (size == 0) {
        size++;
      }
      continue;
...
std::wstring to_wstring(const std::string& s) {
  ...
  uint32_t codepoint = 0;
  while (EatCodePoint(s, i, &i, &codepoint)) {
    // LinuxではwstringはUTF32でエンコードされています:
    if constexpr (sizeof(wchar_t) == 4) {
      out.push_back(codepoint);  // NOLINT
      continue;
    }

    // Windowsでは、wstringはUTF16でエンコードされています:

    // 1ワードを使用してエンコードされたコードポイント:
    // NOLINTNEXTLINE
