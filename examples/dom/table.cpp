...
int main() {
  ...
  // Añadir borde alrededor de la primera columna.
  table.SelectColumn(0).Border(LIGHT);

  // Hacer la primera fila en negrita con un borde doble.
  table.SelectRow(0).Decorate(bold);
  table.SelectRow(0).SeparatorVertical(LIGHT);
  table.SelectRow(0).Border(DOUBLE);

  // Alinear a la derecha la columna "Release date".
  table.SelectColumn(2).DecorateCells(align_right);

  // Seleccionar fila desde la segunda hasta la última.
  auto content = table.SelectRows(1, -1);
  // Alternar entre 3 colores.
