...
/// @brief Un adaptateur. Possède ou référence un objet immuable.
template <typename T>
class ConstRef {
 public:
  ConstRef() = default;
  ...
  // Crée une référence "réaffectable"
...
/// @brief Un adaptateur. Possède ou référence un objet mutable.
template <typename T>
class Ref {
 public:
  Ref() = default;
...
/// @brief Un adaptateur. Possède ou référence une chaîne constante. Par commodité, cette
/// classe convertit plusieurs chaînes mutables vers une représentation partagée.
...
/// @brief Un adaptateur. Possède ou référence une chaîne constante. Par commodité, cette
/// classe convertit plusieurs chaînes immuables vers une représentation partagée.
...
/// @brief Un adaptateur. Référence une liste de chaînes de caractères.
///
/// Entrée supportée :
...
class FTXUI_EXPORT(SCREEN) ConstStringListRef {
 public:
  // Apportez votre propre adaptateur :
  class Adapter {
   public:
    Adapter() = default;
 ...
 private:
  struct IndexedGetter {
    ...
    std::string_view operator()(
      ...
      return "";  // Correctif temporaire : impossible de retourner une vue vers une conversion
                  // temporaire.
    }
