...
/// @brief Representa un evento. Puede ser un evento de pulsación de tecla, un
/// redimensionamiento de terminal, o más...
///
/// Por ejemplo:
...
/// - Se puede encontrar un código arbitrario para eventos especiales usando:
///   ./example/util/print_key_press
///  Por ejemplo, CTRL+A se mapea a Event::Special({1});
///
/// Documentación útil sobre la especificación de xterm:
...
struct FTXUI_EXPORT(COMPONENT) Event {
  ...
  static Event CursorPosition(std::string_view, int x, int y);  // Interno
  static Event CursorShape(std::string_view, int shape);        // Interno
